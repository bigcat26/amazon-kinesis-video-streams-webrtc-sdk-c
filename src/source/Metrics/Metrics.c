...
STATUS getDataChannelStats(PRtcPeerConnection pRtcPeerConnection, PRtcDataChannelStats pRtcDataChannelStats)
{
    ...
    DLOGD("Report: BR: %llu, BS:%llu, MR:%d, MS:%d, name:%s, state:%d\n", pRtcDataChannelStats->bytesReceived, pRtcDataChannelStats->bytesSent,
          pRtcDataChannelStats->messagesReceived, pRtcDataChannelStats->messagesSent, pRtcDataChannelStats->label, pRtcDataChannelStats->state);
CleanUp:
    return retStatus;
}
